...
\begin{document}
    ...
    \section{Преобразование уравнений Лагранжа в уравнения Гамильтона. Обобщенные импульсы. Явный вид функции Гамильтона для уравнений механики.}
     ...
     \hyperlink {lects.15}{Лекции} \\
     	...
     	$$
     		\frac{d}{dt}\left( \frac{\partial L}{\partial \dot{\bfq}} \right) - \frac{\partial L}{\bfq} = 0, \ \ \ \bfq \in \mathbb{R}^n, \ \ \ L=L(\dot{\bfq},\bfq,t)
     	$$
     	при условии
     	$$
     		det \frac{\partial^2 L}{\partial \dot{\bfq}^2} \ne 0
     	$$
     	...
     	$$
     		\left\{
     			\begin{array}{l}
     				\dot{\bfq} = \bfnu \\
     				\frac{d}{dt}\left( \frac{\partial L}{\partial \bfnu} \right) - \frac{\partial L}{\bfq} = 0 \\
     			\end{array}
     		\right. \ \ \ \ \ \ \ \ \ \ (*)
     	$$
     	где $L = L(\bfnu, \bfq, t)$. Далее в фазовом пространстве $(\bfq,\bfnu)$ вводим координаты $(\bfq,\bfp)$, приняв $\bfp = \frac{\partial L}{\partial \bfnu}$. Тогда неравенство $det \frac{\partial^2 L}{\partial \dot{\bfq}^2} \ne 0$ позволяет воспользоваться теоремой о неявной функции и выразить 
     	$$
     	...
     	Для уравнений механики $L = T_2 + T_1 + T_0 - V = \frac{1}{2}\dot{\bfq}^T A \dot{\bfq} + \textbf{a}^T \dot{\bfq} + T_0 - V$ имеем $\frac{\partial^2 L}{\partial \bfnu^2} = A(\bfq, t), \ det \frac{\partial^2 L}{\partial \dot{\bfq}^2} > 0$. Тогда замена 
     	$$
     		\bfnu = \bfphi (\bfq, \bfp, t), \ \ \ \ \ \ \ \bfp = \frac{\partial L}{\partial \bfnu}
     	$$
     	примет вид 
...
\hyperlink {lects.35}{Пункты про отделение переменных} \\
    ...
    \begin{defn}
    Семейство кривых $\gamma_\alpha$ : $\bfz = \bfz_\alpha(t)$, $|\alpha| \leq \alpha_0$ назовем вариацией кривой $\gamma \in \Omega$, если  $\gamma_\alpha \in \Omega, \gamma_0 = \gamma$. \\
    На множестве $\Omega$ определен функционал действия $A(\gamma) = \int^{t_1}_{t_0} \zeta (\dot{\bfz}, \bfz, t)dt, \zeta = \bfp^T\dot{\bfq} - H(\bfz, t) $.
    \end{defn}
    
    \begin{defn}
    Кривую $\gamma$  назовем  экстремалью функционала действия на множестве $\Omega$, если $\frac{d}{d\alpha} A(\gamma_\alpha)|_{\alpha=0}=0$ для любой ее вариации $\gamma_\alpha$.
        \end{defn}
        
   \begin{theorem}(\hyperlink {lects.44}{Теорема внизу страницы})\\
   Кривая $\gamma \in \Omega, \gamma: \bfz=\bfz(t)$ \--- экстремаль функционала действия на множестве $\Omega \Leftrightarrow \bfz(t)$ \--- решение уравнений Гамильтона с функцией Гамильтона $H(\bfz, t)$.
   \end{theorem}
